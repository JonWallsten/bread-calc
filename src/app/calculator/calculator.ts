...
export class CalculatorComponent implements OnInit {
    ...
    readonly mixingMethod = signal<CalcInputs['mixingMethod']>(DEFAULT_INPUTS.mixingMethod);
    ...
    getInputs(): CalcInputs {
        return {
            ...
            flourBlendAdjustment: this.blend.blendValid() ? this.blend.flourBlendAdjustment() : 0,
    ...
    recipeName(recipe: Recipe): string {
        if (recipe.nameKey) {
            const t = this.i18n.t();
            return (
                ((t as unknown as Record<string, unknown>)[recipe.nameKey] as string) ?? recipe.name
            );
        }
        return recipe.name;
    }
