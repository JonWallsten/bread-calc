...
export class RecipeService {
    ...
    readonly allRecipes = computed(() => [...BUILT_IN_RECIPES, ...this.userRecipes()]);
    ...
    readonly uploadableRecipeCount = computed(
        () => this.userRecipes().filter((r) => !r.id.startsWith('cloud-') && !r.skipUpload).length,
    );
    ...
    updateRecipe(id: string, inputs: CalcInputs): void {
        const now = new Date().toISOString();
        this.userRecipes.update((list) =>
            list.map((r) => (r.id === id ? { ...r, inputs: { ...inputs }, updatedAt: now } : r)),
        );
        this.persistUserRecipes();
    ...
    async uploadRecipe(id: string): Promise<void> {
        ...
        try {
            ...
            this.userRecipes.update((list) =>
                list.map((r) =>
                    r.id === id ? { ...r, id: cloudId, updatedAt: now, skipUpload: undefined } : r,
                ),
            );
            if (this.activeRecipeId() === id) {
                this.activeRecipeId.set(cloudId);
    ...
    skipUploadForLocalRecipes(): void {
        this.userRecipes.update((list) =>
            list.map((r) =>
                !r.id.startsWith('cloud-') && !r.skipUpload ? { ...r, skipUpload: true } : r,
            ),
        );
        this.persistUserRecipes();
    }
    ...
    clearCloudRecipes(): void {
        ...
        this.userRecipes.update((list) => list.filter((r) => !r.id.startsWith('cloud-')));
    ...
    async syncToCloud(uploadLocal = true): Promise<void> {
        ...
        try {
            ...
            const cloudMap = new Map<string, { recipe: Recipe; apiUpdatedAt: string }>();
