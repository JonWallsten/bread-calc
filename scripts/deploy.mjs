...
const PROD_API = 'https://jonwallsten.com/bread-calc/api';

async function smokeTest() {
    console.log('🧪  Running post-deploy smoke test...');
    const checks = [
        { name: 'GET /auth/config → 200', path: '/auth/config', expect: 200 },
        { name: 'GET /recipes (no auth) → 401', path: '/recipes', expect: 401 },
        { name: 'GET /sessions (no auth) → 401', path: '/sessions', expect: 401 },
    ];
