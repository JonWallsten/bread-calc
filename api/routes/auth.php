...
function handleAuthRoutes(string $method, string $path): void
{
    ...
    if ($method === 'POST' && $path === '/auth/google') {
        ...
        $stmt = $db->prepare('SELECT id, email, name, picture_url FROM users WHERE google_id = :gid');
    ...
    if ($method === 'GET' && $path === '/auth/me') {
        ...
        $db = getDb();
        $stmt = $db->prepare('SELECT id, email, name, picture_url FROM users WHERE id = :id');
